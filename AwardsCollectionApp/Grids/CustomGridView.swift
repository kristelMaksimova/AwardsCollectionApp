...
struct CustomGridView<Content, T>: View where Content: View {
    ...
    var body: some View {
        ...
        let sideLenght = geometry.size.width / CGFloat(columns)
            ScrollView {
                VStack {
                        ...
                        HStack {
                            ForEach(0..<columns) {columnIndex in
                                if let index = getIndexFor(row: rowIndex, column: columnIndex) {
                                    content(sideLenght, items[index])
                                } else {
                                    Spacer()
                                }
                            }
...
struct CustomGridView_Previews: PreviewProvider {
    static var previews: some View {
        CustomGridView(items: [11, 3, 7, 17, 5, 2, 1], columns: 3) { sideLenght, item in
            VStack {
                Text("\(item)")
            }
            .padding()
            .frame(width: sideLenght, height: sideLenght)
        }
    }
}
